...
class XOS:
    ...
    def get_next_proxy_for_account(self, account_id):
        if account_id not in self.account_proxies:
            if not self.proxies:
                return None
    ...
    def rotate_proxy_for_account(self, account_id):
        if not self.proxies:
            return None
        proxy = self.check_proxy_schemes(self.proxies[self.proxy_index])
        self.account_proxies[account_id] = proxy
    ...
    async def validate_token(self, token: str, proxy=None):
        user_data = await self.user_data(token, proxy)
        if user_data:
            self.log(
                f"{Fore.CYAN + Style.BRIGHT}Status  :{Style.RESET_ALL}"
                f"{Fore.GREEN + Style.BRIGHT} Token Valid {Style.RESET_ALL}"
            )
            return True
        else:
            self.log(
                f"{Fore.CYAN + Style.BRIGHT}Status  :{Style.RESET_ALL}"
                f"{Fore.RED + Style.BRIGHT} Token Invalid or Expired {Style.RESET_ALL}"
            )
            return False

    async def process_accounts(self, token: str, account_id: str, use_proxy: bool):
        proxy = self.get_next_proxy_for_account(account_id) if use_proxy else None
        
        # Validate token
        is_valid = await self.validate_token(token, proxy)
        if not is_valid:
            return
            
        self.log(
            f"{Fore.CYAN + Style.BRIGHT}Proxy   :{Style.RESET_ALL}"
            f"{Fore.WHITE + Style.BRIGHT} {proxy or 'No Proxy'} {Style.RESET_ALL}"
        )
    ...
    async def main(self):
        try:
            ...
            while True:
                ...
                self.welcome()
                self.log(
                    f"{Fore.GREEN + Style.BRIGHT}Token's Total: {Style.RESET_ALL}"
                ...
                for idx, token in enumerate(tokens):
                    if token:
                        ...
                        self.log(
                            ...
                            f"{Fore.WHITE + Style.BRIGHT} Token {idx+1}: {self.mask_token(token)} {Style.RESET_ALL}"
                            f"{Fore.CYAN + Style.BRIGHT}]{separator}{Style.RESET_ALL}"
                        )
                        await self.process_accounts(token, account_id, use_proxy)
